      ...
      module Runners
        module Maintenance
          ...
          def check_corroboration(**) # rubocop:disable Metrics/CyclomaticComplexity
            ...
            candidates = Legion::Data::Model::ApolloEntry.where(status: 'candidate').exclude(embedding: nil).limit(100).all
            confirmed = Legion::Data::Model::ApolloEntry.where(status: 'confirmed').exclude(embedding: nil).limit(500).all
